        ...
        <div className="animate-fadeIn w-full h-full flex flex-col gap-3 pr-1 pb-1 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col xl:flex-row gap-3 min-h-0">
                ...
                <section className="flex-1 bg-gray-900/60 border border-white/10 rounded-xl p-4 flex flex-col min-w-0">
                    <h3 className="text-base font-display text-white uppercase italic font-bold mb-3 flex items-center gap-2 shrink-0">
                    ...
                    <div className="flex-1 grid grid-cols-3 gap-2 min-h-0 items-center">
                        {weapons.map((weapon, index) => (
                            <div key={index} className="h-full bg-black/40 border border-white/5 rounded-lg p-2 flex flex-col items-center justify-center gap-2 hover:border-white/20 transition-colors group min-w-0">
                ...
                <section className="flex-1 bg-gray-900/60 border border-white/10 rounded-xl p-4 flex flex-col min-w-0">
                    ...
                    <div className="flex-1 flex flex-row gap-3 min-h-0 items-center">
                        ...
                        <div className="flex-1 h-full bg-black/40 border border-white/5 rounded-lg p-2 flex items-center justify-center gap-2 min-w-0">
                            {driveDiscs.sets.map((set, index) => (
                                <div key={index} className="flex items-center gap-1 min-w-0">
                                    <div className="relative w-20 h-20 shrink-0 group" title={set.name}>
                                        <Image
                                    ...
                                    {index === 0 && driveDiscs.sets.length > 1 && (
                                        <div className="text-gray-500 text-2xl font-bold shrink-0">+</div>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex-[1.2] flex flex-col justify-center gap-2 min-w-0">
                            <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded border border-white/5">
                                <span className="text-xs font-bold text-gray-400 w-6 shrink-0">IV</span>
                                <span className="text-xs text-white font-medium text-right truncate">{driveDiscs.stats.main.iv}</span>
                            </div>
                            <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded border border-white/5">
                                <span className="text-xs font-bold text-gray-400 w-6 shrink-0">V</span>
                                <span className="text-xs text-white font-medium text-right truncate">{driveDiscs.stats.main.v}</span>
                            </div>
                            <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded border border-white/5">
                                <span className="text-xs font-bold text-gray-400 w-6 shrink-0">VI</span>
            ...
            <section className="shrink-0 bg-gray-900/60 border border-white/10 rounded-xl p-4 min-w-0">
                <h3 className="text-base font-display text-white uppercase italic font-bold mb-3 flex items-center gap-2">
                    <div className="w-1.5 h-5 bg-yellow-500 rounded-full"></div>
                    Composiciones
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {teams.map((team, index) => (
                        <div key={index} className="bg-black/40 border border-white/5 rounded-lg p-3 flex items-center justify-between gap-3 min-w-0">
                            ...
                            <div className="flex items-center gap-3 min-w-0 overflow-x-auto no-scrollbar">
                                {team.members.map((member, mIdx) => (
                                    <div key={mIdx} className="flex flex-col items-center group cursor-pointer relative shrink-0">
                                        <div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-gray-600 group-hover:border-yellow-500 transition-colors bg-gray-800">
                                            <Image
                                                src={member.icon}
                                                alt={member.name}
                                                fill
