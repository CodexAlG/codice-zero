...
export default function WikiHome() {
  // Filtrar agentes que no tengan leak: "Beta"
  const releaseAgents = agents.filter(agent => agent.leak !== "Beta");

  // Obtener la última versión del juego desde los agentes lanzados
  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col relative overflow-hidden px-4 md:px-8 py-12">
      {/* Fondo de Malla (Grid) */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] pointer-events-none"></div>

      {/* Contenido Principal */}
      <main className="flex-1 flex flex-col items-center justify-center w-full max-w-4xl text-center mx-auto relative z-10">
